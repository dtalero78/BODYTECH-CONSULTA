...
**Solo para la coordinación de la UMV** (pedido el 10-sep-2026). Además del `requireRole('admin', 'coordinador')`, todas las rutas de `/api/digitalizar` exigen que el correo esté en `DIGITALIZAR_PERMITIDOS` (correos separados por coma, [digitalizar-acceso.ts](backend/src/services/digitalizar-acceso.ts)) y le responden 403 a cualquier otro coordinador. Va por lista y no por programa porque el alcance de la sesión es por **sede**: "coordinador de la UMV" no se puede deducir de ella. Sin la variable —o con la variable vacía— entra solo el autor, nunca todos. El panel no tiene copia de la lista: le pregunta a `GET /api/digitalizar/acceso`, que va antes del candado, si dibuja la pestaña.
