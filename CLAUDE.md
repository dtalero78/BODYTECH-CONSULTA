...
Flujo: el webhook `/grabacion` dispara `transcribirGrabacion()` → baja el MP3 de Twilio → lo sube a `RECORDINGS_BUCKET` bajo `audio-llamada/` → arranca el job. Un barrido cada 10 min sondea los jobs en curso y guarda el resultado; **el audio se borra de S3 al terminar** (es PHI). El claim del `UPDATE … RETURNING` evita arrancar el job dos veces. `POST /api/twilio/llamadas/:id/transcribir` (auditoría) lo rehace.
