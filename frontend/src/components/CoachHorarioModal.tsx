...
interface Props {
  ...
  /** Nombre/código del coach para mostrar en el header (opcional). */
  subtitulo?: string;
}
...
/**
 * Modal para que el PROPIO coach/médico fije su horario (disponibilidad
 * recurrente) desde el panel médico. Réplica del modal del coordinador, pero
...
export function CoachHorarioModal({ isOpen, onClose, onSaved, onError, subtitulo }: Props) {
    ...
    profesionalesService
      ...
      .catch((err) => {
        ...
        const msg =
          ...
          (err as any)?.response?.data?.error?.message || 'Error cargando tu horario.';
        onError(msg);
      })
      .finally(() => {
  ...
  function toggleDia(dia: number) {
    setDiasState((prev) => {
      ...
      if (current.activo) return { ...prev, [dia]: { activo: false, rangos: [] } };
  ...
  function removeRango(dia: number, idx: number) {
    setDiasState((prev) => {
      ...
      return { ...prev, [dia]: { activo: newRangos.length > 0, rangos: newRangos } };
    });
  }
  ...
  function copiarATodos(diaFuente: number) {
    setDiasState((prev) => {
      ...
      for (const d of DIAS) {
        ...
        next[d.id] = { activo: true, rangos: fuente.rangos.map((r) => ({ ...r })) };
      }
      return next;
    });
  }
  ...
  async function handleGuardar() {
    ...
    for (const d of DIAS) {
      ...
      if (!state.activo || state.rangos.length === 0) continue;
    ...
    setSaving(true);
    try {
      await profesionalesService.replaceMiDisponibilidad(modalidad, dias);
      onSaved();
    } catch (err: unknown) {
      const msg =
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex justify-end">
      <div className="bg-white w-full max-w-xl h-full overflow-y-auto shadow-xl">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-lg font-semibold text-gray-800">Mi horario</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              {subtitulo ? `${subtitulo} · ` : ''}Disponibilidad semanal
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-3">
          {loading ? (
            <p className="text-sm text-gray-500 text-center py-8">Cargando tu horario...</p>
          ) : (
            DIAS.map((d) => {
              const state = diasState[d.id];
              return (
                <div
                  ...
                  className={`rounded-lg border ${
                    state.activo ? 'border-green-200 bg-green-50/40' : 'border-gray-200'
                  }`}
                >
                  ...
                  {state.activo && (
                    <div className="px-3 pb-3 space-y-2">
                      ...
                      <button
                        ...
                        className="text-xs text-green-700 hover:text-green-800 flex items-center gap-1 mt-1"
                      >
