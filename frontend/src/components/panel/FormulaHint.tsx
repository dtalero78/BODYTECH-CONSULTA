    ...
    >
      <button
        ...
        className={`w-[34px] h-[34px] rounded-[10px] grid place-items-center border transition ${
          open
            ? 'bg-[rgba(var(--p-accent-rgb),0.18)] border-[rgba(var(--p-accent-rgb),0.50)] text-[var(--p-accent)]'
            : 'bg-[rgba(var(--p-accent-rgb),0.08)] border-[var(--p-line)] text-[var(--p-text-2)] hover:text-[var(--p-accent)] hover:border-[rgba(var(--p-accent-rgb),0.40)]'
        }`}
      >
      ...
      {open && (
        <div
          role="tooltip"
          className="absolute right-0 top-[calc(100%+8px)] z-50 w-[420px] max-w-[80vw] rounded-[14px] border border-[var(--p-line-2)] bg-[var(--p-surface-2)] shadow-2xl p-4"
          style={{ animation: 'panelScaleY 140ms ease-out', transformOrigin: 'top right' }}
        >
          <div className="text-[10.5px] font-semibold text-[var(--p-text-3)] tracking-widest uppercase mb-2.5">
            Cómo se calcula
          </div>
          <div className="flex flex-col gap-2.5">
            {formulas.map((f) => (
              <div key={f.campo} className="flex flex-col gap-1">
                <div className="text-[12.5px] font-semibold text-[var(--p-text)]">{f.campo}</div>
                <div className="text-[12px] text-[var(--p-accent)] font-mono leading-snug break-words">
                  {f.formula}
                </div>
                {f.nota && (
                  <div className="text-[11px] text-[var(--p-text-2)] leading-snug">{f.nota}</div>
                )}
              </div>
            ))}
          </div>
